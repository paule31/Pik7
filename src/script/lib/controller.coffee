...
# A controller instance connects many emitters and provides an api to control
# the whole application. It's an umbrealla for:
...
define [
  ...
  'lib/hash',
  'lib/controls'
], (State, Sync, Hash, Controls) -> return class Controller
  ...
  # Create the four emitters (the state object with a starting state derived
  # from the defaults, the url and local storage), then connect emitters
  createEmitters: (defaults) ->
  ...
  # 3. `@state.update()` triggers the controller, if the update did really set
  #    new values
  # Passing emitters as arguments simplifies connecting emitters from
  # elsewhere (eg. the frames' controls instances)
  ...
  connectHash: (emitter) ->
    ...
    onstatechange = (key, value) =>
      # TODO: `update` should better be called `setState`, like in Sync
      emitter.update(key, value, {
        file: @state.get('file')
        slide: @state.get('slide')
        hidden: @state.get('hidden')
      })
  ...
  connectSync: (emitter) ->
    ...
    emitter.on 'change', (state) =>
      # **HACK HACK HACK HACK**
      # If we use the value passed to this callback to set the state, the
      # state is changed one value at a time. This would only work for changes
      # that only touch one of the state's properties - if more properties are
      # changed after one another, it gets nasty as soon more than one window
      # is involved. What happens is that when we update the state
      # `{ a:x, b:1 }` to `{ a:y, b:1 }` and then to `{ a:y, b:2 }`, the
      # storage event for `{ a:y, b:1 }` is triggered on other windows *after*
      # we set `{ a:y, b:2 }` on our window. So the other window triggers an
      # event for `{ a:y, b:1 }`, while we are already on `{ a:y, b:2 }`, so
      # we update and trigger an event, while the other window recieves
      # `{ a:y, b:2 }` ... and we've got ourselfes a nice endless loop. This
      # can be "fixed" by not actually setting the state to the value that
      # the Sync callback reports, but to the value that's currently in the
      # sync's storage. This works because storage events *always* fire only
      # after all changes to the storage are applied, even for multiple
      # changes in sequence. This allows us to fake a bulk update of the
      # state, which fixes the endless loop problem.
  ...
  # of the SmartEmitter's functionality by using `null` for caller/subscriber
  # objects
  on: (args...) ->
    # add `null` as the `subscriber` object
    args.splice(1, 0, null) if args.length == 2
    @state.on(args...)
  trigger: (args...) ->
    # add `null` as the `caller` object
    args.splice(1, 0, null) if args.length == 2
    @state.trigger(args...)
  off: (args...) ->
    @state.off(args...)
