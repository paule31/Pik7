...
# A simple event emitter with an internal, persistent state as well as
# `get()` and `set()` methods.

define ['lib/emitter'], (Emitter) -> return class StatefulEmitter extends Emitter
  ...
  constructor: (stateObj, @storageKey) ->
    ...
    if not state?
      throw new Error "Failed to create state, no stored state and no default
        state found"
  ...
  # Compare current state with local storage and trigger state changes when
  ...
  trigger: (key, val, rest...) ->
    if not val?
      throw new Error "StatefulEmitter requires a value for triggers,
        got #{typeof val}"
    if rest.length > 0
      throw new Error "StatefulEmitter can only pass one value,
        got #{rest.length + 1}"
    super(key, val)
