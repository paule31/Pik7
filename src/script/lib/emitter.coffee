...
define -> return class Emitter
  ...
  on: (topic, callback) ->
    if typeof callback != 'function'
      throw new Error "Can't add callback to '#{topic}', '#{callback}' is not
        a function"
  ...
  off: (topic, callback) ->
    if typeof callback != 'function'
      throw new Error "Can't remove callback from '#{topic}', '#{callback}' is
        not a function"
    else if topic not of @topics
  ...
  offAll: (topic) ->
    removeAllCallbacks = (list, target) ->
      if target of list
        ...
        throw new Error "Can't remove callbacks from non-existant
          topic '#{target}'"
    if topic?
